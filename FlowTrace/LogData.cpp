...
CHAR* LOG_NODE::getTreeText(int* cBuf, bool extened)
{
    ...
    else if (isApp())
    {
        ...
        if (This->pid > 0) //&& gSettings.GetColPID()
        {
    ...
    else if (isThread())
    {
        ...
        pBuf[cb] = 0;
        if (getPid() > 0) // && gSettings.GetColPID()
        {
            cb += _sntprintf_s(pBuf + cb, cMaxBuf - cb, cMaxBuf - cb, TEXT(" [%d-%d]"), getPid(), getTid());
        }
...
CHAR* LOG_NODE::getListText(int *cBuf, LIST_COL col)
{
    ...
    else if (col == PID_COL && getPid() > 0)
    {
        if (gSettings.GetColPID())
            cb += _sntprintf_s(pBuf, MAX_BUF_LEN, MAX_BUF_LEN, TEXT("[%d-%d]"), getPid(), getTid());
	}
