...
APP_NODE* Archive::addApp(char* app_name, int cb_app_name, DWORD app_sec, DWORD app_msec, DWORD nn, sockaddr_in *p_si_other)
{
    APP_NODE* pNode = (APP_NODE*)(node_array - c_rec);
    APP_DATA* pAppData = (APP_DATA*)rec_end;
    rec_end = rec_end + sizeof(APP_DATA) + cb_app_name;

    pAppData->init();
    pAppData->data_type = APP_DATA_TYPE;
    pAppData->app_sec = app_sec;
    pAppData->app_msec = app_msec;
    pAppData->cb_app_name = cb_app_name;
    pAppData->NN = nn;
    pAppData->threadCount = 0;
    pAppData->lost = 0;
    pAppData->ip_address[0] = 0;
    memcpy(pAppData->appName(), app_name, cb_app_name);
    pNode->init(pAppData);
...
APP_NODE* Archive::getApp(ROW_LOG_REC* p, sockaddr_in *p_si_other)
{
    if (curApp)
    {
        ...
        if (curApp && (0 == memcmp(pAppData->appName(), p->appName(), p->cb_app_name)) && (pAppData->app_sec == p->app_sec && pAppData->app_msec == p->app_msec))
            return curApp;
    }
    ...
    while (curApp)
    {
        APP_DATA* pAppData = curApp->getData();
        //pAppData->Log(); p->Log();
        if ((0 == memcmp(pAppData->appName(), p->appName(), p->cb_app_name)) && (pAppData->app_sec == p->app_sec && pAppData->app_msec == p->app_msec))
            break;
    ...
    if (!curApp)
    {
        curApp = addApp(p->appName(), p->cb_app_name, p->app_sec, p->app_msec, p->nn, p_si_other);
        //curApp->getData()->Log();
    }
