...
bool WideToUTF8(const wchar_t* utf16, const size_t size, std::string* utf8) {
  ...
  const int chars_required = WideCharToMultiByte(CP_UTF8, flags, utf16, (int)size,
                                                 NULL, 0, NULL, NULL);
  if (chars_required <= 0) {
  ...
  const int result = WideCharToMultiByte(CP_UTF8, flags, utf16, (int)size,
                                         &(*utf8)[0], chars_required, NULL,
                                         NULL);
  if (result != chars_required) {
    SetErrnoFromLastError();
	D("WideCharToMultiByte wrote %d chars to buffer of %d chars", result, chars_required);
    utf8->clear();
    return false;
  }
...
static bool UTF8ToWideWithFlags(const char* utf8, const size_t size, std::wstring* utf16, const DWORD flags) {
  ...
  // TODO: Consider using std::wstring_convert once libcxx is supported on
  // Windows.
  const int chars_required = MultiByteToWideChar(CP_UTF8, flags, utf8, (int)size,
                                                 NULL, 0);
  if (chars_required <= 0) {
    SetErrnoFromLastError();
    return false;
  ...
  const int result = MultiByteToWideChar(CP_UTF8, flags, utf8, (int)size,
                                         &(*utf16)[0], chars_required);
  if (result != chars_required) {
    SetErrnoFromLastError();
	D("MultiByteToWideChar wrote %d chars to buffer of %d chars", result, chars_required);
