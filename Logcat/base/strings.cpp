...
//bool StartsWith(const std::string_view& s, const char* prefix) {
...
bool StartsWithIgnoreCase(const std::string& s, const char* prefix) {
  return _strnicmp(s.c_str(), prefix, strlen(prefix)) == 0;
}
...
static bool EndsWith(const std::string& s, const char* suffix, size_t suffix_length,
  ...
  return (case_sensitive ? strncmp : _strnicmp)(s.c_str() + offset, suffix, suffix_length) == 0;
}
