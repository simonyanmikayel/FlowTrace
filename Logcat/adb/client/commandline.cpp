...
int read_and_dump(int fd, bool use_shell_protocol = false,
    ...
    if (use_shell_protocol) {
        ...
        raw_buffer = new char[MAX_PAYLOAD];
        buffer_ptr = raw_buffer;        
    }
        ...
        if (use_shell_protocol) {
            ...
            switch (protocol->id()) {
                case ShellProtocol::kIdStdout:
                    buffer_ptr[length] = 0;
                    continueLoop = callback->OnStdout(buffer_ptr, length);
                    break;
                case ShellProtocol::kIdStderr:
                    buffer_ptr[length] = 0;
                    continueLoop = callback->OnStderr(buffer_ptr, length);
                    break;
            ...
            length = adb_read(fd, buffer_ptr, MAX_PAYLOAD - 1);
...
void copy_to_file(int inFd, int outFd) {
    ...
    while (true) {
        if (inFd == STDIN_FILENO) {
            ...
            len = adb_read(inFd, buf.data(), (int)buf.size());
        }
        if (len == 0) {
...
static int logcat(int argc, const char** argv, StandardStreamsCallbackInterface* callback = nullptr) {
    ...
    return send_shell_command(cmd, false, callback ? callback : &DEFAULT_STANDARD_STREAMS_CALLBACK);
}
